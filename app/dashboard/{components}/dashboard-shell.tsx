...
import ParchmentLogo from '../../{components}/parchment-logo'

const sidebarNavigation = [
  { name: 'Home', href: DASHBOARD_LINK, icon: HomeIcon, current: false },
  {
    name: 'Patients',
    href: PATIENTS_LINK,
    icon: Squares2X2Icon,
...
export default function DashboardShell({
  ...
  const { logout } = useContext(AmplifyUserContext)
    ...
    <>
      ...
      */}
      <div className="flex h-screen">
        {/* Narrow sidebar */}
        <div className="hidden w-28 overflow-y-auto bg-primary-700 md:block">
        ...
        <div className="flex flex-1 flex-col ">
          ...
          <div className=" mt-2 overflow-y-auto px-2 sm:mt-7 sm:px-6 lg:px-8">
            {/* Main content */}
            {children}
          </div>
        </div>
...
function ProfileDropdown({ user, logout }: { user: User; logout: any }) {
  return (
    <Menu as="div" className="relative flex-shrink-0">
      <div>
        <Menu.Button className="flex rounded-full bg-white text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2">
          <span className="sr-only">Open user menu</span>
          <Avatar full_name={user?.full_name} />
        </Menu.Button>
      </div>
      <Transition
        as={Fragment}
